...
!!! info
    The style guide is always a work in progress, and not all LinearElasticityBase code
    follows the rules. When modifying LinearElasticityBase, please fix the style violations
...
LinearElasticityBase uses [JuliaFormatter](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/MineralsCloud/LinearElasticityBase.jl/blob/main/.JuliaFormatter.toml).
...
!!! info
    A continuous integration check verifies that all PRs made to LinearElasticityBase have
