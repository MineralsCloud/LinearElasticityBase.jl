...
[LinearElasticityBase](https://github.com/MineralsCloud/LinearElasticityBase.jl).
...
2. Run the following commands and wait for them to finish:
   ...
   julia> Pkg.add("LinearElasticityBase")
   ```
...
version of LinearElasticityBase, type

```@repl
using Pkg
Pkg.update()
pkg"add https://github.com/MineralsCloud/LinearElasticityBase.jl"
```
...
our [GitHub repository](https://github.com/MineralsCloud/LinearElasticityBase.jl)
for new releases.
Once we release a new version, you can update LinearElasticityBase by typing

```@repl
using Pkg
Pkg.update("LinearElasticityBase")
Pkg.gc()
```
...
1. To uninstall, in a Julia session, run
   ...
   julia> Pkg.rm("LinearElasticityBase")

   julia> Pkg.gc()
   ```

2. Press `ctrl+d` to quit the current session. Start a new Julia session and
   reinstall LinearElasticityBase.
